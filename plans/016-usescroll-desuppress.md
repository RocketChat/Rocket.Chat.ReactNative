# 016 — Make `useScroll.ts` compile: honest deps + latest-closure ref for the re-observe effect

- **Status:** DONE `535cc4920`
- **Priority:** P1
- **Effort:** S
- **Risk:** Low-Medium (subtle jump-to-message machinery, but target content is pre-verified to compile/lint/format clean and 18 behavior tests pin the contract)
- **Planned at:** `8ee630b06`
...
**The problem:** `app/views/RoomView/List/hooks/useScroll.ts` carries `'use memo'` but the compiler silently skips it because of its 2 `// eslint-disable-next-line react-hooks/exhaustive-deps` comments (the compiler bails on exhaustive-deps suppressions). Its consumer `ListContainer` (`app/views/RoomView/List/index.tsx`) IS compiled, so `useScroll`'s un-memoized returns (`jumpToBottom`, `handleScrollToIndexFailed`, …) get fresh identities every render and defeat downstream memoization.

**The fix (target content below is pre-verified):**

1. **Release effect** (`pendingBottom`): add `listRef` to the deps (`[messages, listRef]`) — `listRef` is a stable ref prop, so semantics are unchanged; the suppression dies. `messages` stays as the intentional trigger dep (eslint allows extra reactive deps).
2. Its `setIsReleasing(false)` call gets a **narrow, documented suppression of `react-hooks/set-state-in-effect`** — that rule only surfaces now because analysis was previously skipped; the cascade is deliberate (the swap render must paint with maintainVisibleContentPosition suppressed, the follow-up render restores it). This suppression does NOT re-trigger the compiler skip (verified: the compiler only bails on `exhaustive-deps` / `rules-of-hooks` suppressions).
3. **Re-observe effect**: its body moves to a component-scope `onReObserve` function, published through a **latest-closure ref** (`onReObserveRef`) refreshed in a dep-less layout effect declared BEFORE the trigger effect (layout effects run in declaration order, so the trigger always reads the current commit's closure). The trigger effect keys on `[messages]` alone. This preserves the exact "one run per re-observe" contract: listing the handler (or the helpers it calls) as deps would either warn ("makes the dependencies change on every render") or re-fire the effect mid-jump, because `fetchMessages` re-keys on `highTs`. Same pattern as `onThreadMessagesLoadedRef` in `app/views/RoomView/hooks/useRoomLifecycle.ts` (NATIVE-1356).
...
### 1. `app/views/RoomView/List/hooks/useScroll.ts` — replace ENTIRE file content with:
...
The diff vs current is ONLY: the release effect's dep array gains `listRef` and its suppression is replaced by the narrow `set-state-in-effect` one (its lead comment gains the "keyed on messages" clause that used to live in the suppression); the re-observe effect body becomes `onReObserve` + `onReObserveRef` + two layout effects (assignment first, `[messages]` trigger second). Every other byte — constants, helpers, `jumpToMessage`, `cancelJumpToMessage`, `handleScrollToIndexFailed`, comments — is unchanged.

### 2. `app/views/RoomView/reactCompilerContract.test.ts`

In `KNOWN_SKIPPED`, remove the `'app/views/RoomView/List/hooks/useScroll.ts'` entry (with its trailing comment). Keep the `useRoomLifecycle.ts` entry — the remaining array is:
...
- **Out of scope — do not touch:** `useScroll.test.tsx` (must pass UNCHANGED — it is the behavior contract), `useMessages.ts`, `List/index.tsx`, `useRoomLifecycle.ts`, `babel.config.js`, eslint config.

## Verification / done criteria

1. `TZ=UTC pnpm test --testPathPattern='reactCompilerContract'` → passes. If it fails naming `useScroll.ts`, STOP and report the compiler error.
2. `TZ=UTC pnpm test --testPathPattern='useScroll'` → all 18 tests pass UNCHANGED. A failure is a REAL regression — STOP and report; do not edit the test.
3. `npx eslint --resolve-plugins-relative-to . . 2>&1 | tail -3` → exactly `✖ 174 problems (0 errors, 174 warnings)` (unchanged baseline: the old file contributed 0 warnings, the new file contributes 0). Any count above 174: STOP, report verbatim.
...
Existing `useScroll.test.tsx` (18 tests: anchored/contiguous jumps, growth retries, safety-net aborts, cancellation, jump-to-bottom release, scroll-to-index retry chains) is the behavior contract and must pass unmodified. The contract test's ratchet is the compile-level proof. No new tests.

## Maintenance note

- New re-observe logic belongs inside `onReObserve`; the trigger effect must stay keyed on `[messages]` alone and the assignment effect must stay declared before it.
- The `set-state-in-effect` suppression is load-bearing: deriving `isReleasing` instead of setting it in the effect would restore MVCP in the same render as the data swap and reintroduce the viewport-drag bug the comment describes.

## Escape hatches

- Each done criterion above carries its STOP condition.
- If the current files at your checkout differ from the pre-edit state described (drift from `8ee630b06`), STOP and report.
