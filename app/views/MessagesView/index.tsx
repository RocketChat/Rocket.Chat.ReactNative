...
import { getFiles, getMessages, togglePinMessage, toggleStarMessage } from '../../lib/services';
	...
	defineMessagesViewContent = (name: string) => {
		...
		return {
			...
			Mentions: {
				...
				fetchFunc: () => {
					const { messages } = this.state;
					return getMessages(this.rid, this.t, { 'mentions._id': { $in: [user.id] } }, messages.length);
				},
			...
			Starred: {
				...
				fetchFunc: () => {
					const { messages } = this.state;
					return getMessages(this.rid, this.t, { 'starred._id': { $in: [user.id] } }, messages.length);
				},
				...
				handleActionPress: (message: IMessageItem) => toggleStarMessage(message._id, message.starred)
			},
			...
			Pinned: {
				...
				fetchFunc: () => {
					const { messages } = this.state;
					return getMessages(this.rid, this.t, { pinned: true }, messages.length);
				},
				...
				handleActionPress: (message: IMessageItem) => togglePinMessage(message._id, message.pinned)
			}
			// @ts-ignore
		}[name];
	};
