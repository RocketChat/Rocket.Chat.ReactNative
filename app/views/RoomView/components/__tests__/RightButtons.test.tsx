...
let mockRoomState = {
	...makeRoomSnapshotState({ rid: 'rid-1', t: 'c', name: 'general' }),
...
describe('RightButtons', () => {
	...
	it('renders nothing for an invited room', () => {
		mockRoomState = { ...mockRoomState, ...makeRoomSnapshotState({ rid: 'rid-1', t: 'c', name: 'general', status: 'INVITED' }) };
	...
	it('renders nothing for a queued omnichannel room', () => {
		mockRoomState = { ...mockRoomState, ...makeRoomSnapshotState({ rid: 'rid-1', t: 'l', name: 'chat', status: 'queued' }) };
		const { queryByTestId, toJSON } = render(<RightButtons rid='rid-1' roomStore={roomStore} />);
		expect(toJSON()).toBeNull();
		expectOnly(queryByTestId, []);
		expect(toJSON()).toMatchSnapshot();
	...
	it('renders only the kebab for an active omnichannel room', () => {
		mockRoomState = { ...mockRoomState, ...makeRoomSnapshotState({ rid: 'rid-1', t: 'l', name: 'chat' }) };
		const { queryByTestId, toJSON } = render(<RightButtons rid='rid-1' roomStore={roomStore} />);
	...
	it('enables the encryption button when the user can toggle encryption', () => {
		mockRoomState = { ...mockRoomState, ...makeRoomSnapshotState({ rid: 'rid-1', t: 'c', name: 'general', encrypted: true }) };
	...
	it('renders the push troubleshoot button when notifications are disabled for the room', () => {
		mockRoomState = {
			...mockRoomState,
			...makeRoomSnapshotState({ rid: 'rid-1', t: 'c', name: 'general', disableNotifications: true })
		};
		const { queryByTestId, toJSON } = render(<RightButtons rid='rid-1' roomStore={roomStore} />);
		expectOnly(queryByTestId, [
			'room-view-push-troubleshoot',
	...
	it('hides the call button on a self DM', () => {
		mockRoomState = { ...mockRoomState, ...makeRoomSnapshotState({ rid: 'rid-1', t: 'd', name: 'user' }) };
