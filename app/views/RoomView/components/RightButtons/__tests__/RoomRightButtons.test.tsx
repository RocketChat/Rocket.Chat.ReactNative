...
let mockRoomState = { room: { room: { rid: 'rid-1', t: 'c', name: 'general' } as Record<string, unknown> } };
jest.mock('zustand', () => ({
...
describe('RoomRightButtons', () => {
	beforeEach(() => {
		...
		mockRoomState = { room: { room: { rid: 'rid-1', t: 'c', name: 'general' } } };
	...
	it('hides the call button on a self DM', () => {
		mockRoomState = { room: { room: { rid: 'rid-1', t: 'd', name: 'user' } } };
	...
	it.each([false, true])('routes disabled Room notifications to preferences (master-detail: %s)', isMasterDetail => {
		mockIsMasterDetail = isMasterDetail;
		mockRoomState = { room: { room: { rid: 'rid-1', t: 'c', name: 'general', disableNotifications: true } } };
		mockUnreads = { ...mockUnreads, subscription: { id: 'rid-1' } };
	...
	it('navigates to the threads and search screens on stack mode', () => {
		mockRoomState = { room: { room: { rid: 'rid-1', t: 'c', name: 'general', encrypted: true } } };
