...
- **Anchored Window** — pinned around a **Jump to Message** target that sits far from the Live Tail. Deliberately does *not* follow new Messages until the user explicitly rejoins live.
...
| Anchor decision (pure) | `services/anchorResolver.ts`, `services/resolveJumpAnchor.ts`, `services/getLocalAnchor.ts` | Decide the `highTs` bound for a jump, and whether to raise or release it — purely from rows/ts, no React, no DB (except `getLocalAnchor`'s single query). Unit-tested with plain objects. |
...
| Orchestration | `views/RoomView/index.tsx` | Resolves the target Message, computes the anchor via the pure resolver, and drives the list's `jumpToMessage`. Owns the jump-from-navigation-param lifecycle. |
...
- **`fetchMessages()`** — grows the *current* window by one page (`count += QUERY_SIZE`) and re-subscribes. Used by scroll-to-end pagination and by the jump growth retry. Does **not** change `highTs`.
...
- The safety net (`JUMP_SAFETY_TIMEOUT = 5000ms`) — aborts only if the target *never* re-observes. Refreshed on each productive growth so a deep, still-loading target is not aborted mid-load. It cannot interrupt a completed scroll: `completeJump` clears it the moment the target appears.
...
- It tracks whether the boundary Newer Loader was present in the previous emit. A **present → absent** transition means `loadNextMessages` consumed it and wrote the next batch + a new Loader *above* the current bound — which the bounded observation (`ts <= highTs`) cannot see.
- On that transition it reads the region above the bound directly and hands it to the pure `raiseOrRelease`:
  - **Newer Loaders remain** ⇒ RAISE: climb the bound to the Loader nearest the Live Tail (monotonic, never moves backward). `count` is left intact so the next re-subscribe *grows* the window by a page and the user's reading position is preserved alongside the newly revealed batch.
...
- **Bounded `ts` window on the existing inverted list (chosen).** The only new capability a centered jump needs is an *upper* bound; the existing growing `take(count)` already handles the lower extent, Loader Rows, `MAX_AUTO_LOADS`, and the `hideSystemMessages` clause.
...
- **Thread jump fires after thread rows load** — a thread jump fires from `init()`'s success path (after `loadThreadMessages`), not `componentDidMount`; firing before the rows exist aborts on the safety net and parks on the Live Tail. The param is read-and-cleared so the other `init()` callers cannot re-fire it. See `RoomView/index.tsx`.
