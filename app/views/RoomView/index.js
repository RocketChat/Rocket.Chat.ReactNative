...
export default class RoomView extends React.Component {
	...
	onReactionPress = (shortname, messageId) => {
		...
		try {
			if (!messageId) {
				return RocketChat.setReaction(shortname, actionMessage._id);
	...
	renderItem = (item, previousItem) => {
		...
		const {
			user, Message_GroupingPeriod, Message_TimeFormat, useRealName, baseUrl, useMarkdown, navigation, toggleReactionPicker
		} = this.props;
		...
		return (
			<React.Fragment>
				{room._id && showActions
					? <MessageActions room={room} tmid={this.tmid} user={user} toast={this.toast} navigation={navigation} />
					: null
				}
