	...
	return (
		<FormContainer
			...
			<FormContainerInner accessibilityLabel={I18n.t('Add_server')}>
				...
				<Text style={{ ...styles.description, color: colors.fontSecondaryInfo, textAlign: 'center' }}>{DEFAULT_SERVER_URL}</Text>
				{failureMessage ? (
					<Text style={{ ...styles.error, color: colors.fontDanger, textAlign: 'center' }}>{failureMessage}</Text>
				) : null}
				<Button
