...
class ProfileView extends React.Component {
  ...
  formIsChanged = () => {
    ...
    if (customFieldsKeys.length) {
      customFieldsKeys.forEach(key => {
        if (
          !user.customFields ||
          user.customFields[key] !== customFields[key]
        ) {
  ...
  submit = async () => {
    // this.setState({ isAvatarUploadActive: false });
    Keyboard.dismiss();
    ...
    try {
      ...
      if (result.success) {
        ...
        EventEmitter.emit(LISTENER, {
          message: I18n.t("Profile_saved_successfully")
        });
        this.init();
      }
    } catch (e) {
  ...
  resetAvatar = async () => {
    try {
      ...
      EventEmitter.emit(LISTENER, {
        message: I18n.t("Avatar_changed_successfully")
      });
      this.init();
    } catch (e) {
