...
class FullScreenComposerView extends Component {
	...
	UNSAFE_componentWillReceiveProps(nextProps) {
		...
		if (editing !== nextProps.route.params.editing && nextProps.route.params.editing) {
			this.setInput(nextProps.route.params.message.msg);
			if (this.text) {
				this.setShowSend(true);
			}
			this.focus();
		} else if (replying !== nextProps.route.params.replying && nextProps.route.params.replying) {
			this.focus();
		} else if (!nextProps.route.params.message) {
			this.clearInput();
		}
	}
	...
	renderBottomBar = () => {
		...
		const {
			showEmojiKeyboard, showSend, commandPreview, showCommandPreview, mentions, trackingType
		} = this.state;
		...
		return (
			<>
				...
				<View style={[styles.buttons, { backgroundColor: themes[theme].messageboxBackground }, editing && { backgroundColor: themes[theme].chatComponentBackground }]}>
					<LeftButtons
						theme={theme}
...
export default connect(mapStateToProps, dispatchToProps, null, { forwardRef: true })(withTheme(FullScreenComposerView));
