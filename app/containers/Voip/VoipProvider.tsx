...
import { VoipContext, VoipContextValue } from './contexts/VoipContext';
...
const VoipProvider = ({ children }: { children: ReactNode }) => {
	...
	const isVoipEnabled = true; // isVoipSettingEnabled && canViewVoipRegistrationInfo;
	...
	const { voipClient, error } = useVoipClient({
		enabled: false, // isVoipEnabled,
		autoRegister: true // isLocalRegistered
	});
	...
	useEffect(() => {
		...
		// 	dispatchToastMessage({ type: 'error', message: i18n.t('Voice_calling_registration_failed') });
		// };
	...
	}, [/* dispatchToastMessage, setStorageRegistered,*/ voipClient]);

	const changeAudioOutputDevice = useCallback(
		async (_selectedAudioDevice: Device): Promise<void> => {
		...
		[
			/* setOutputMediaDevice*/
		]
	);

	const changeAudioInputDevice = useCallback(
		async (selectedAudioDevice: Device): Promise<void> => {
			if (!voipClient) {
				return;
			}
		...
		[/* setInputMediaDevice,*/ voipClient]
	);
	...
	}, [isVoipEnabled, voipClient, error, changeAudioInputDevice, changeAudioOutputDevice]);
