...
const Markdown = ({ msg, customEmojis }) => {
  ...
  const rules = {
    username: {
      ...
      react: (node, output, state) => ({
        ...
        props: {
          children: (
            <Text key={state.key} style={mentionStyle} onPress={() => alert('Username')}>
              {node.content}
            </Text>
          ),
        },
    ...
    heading: {
      ...
      react: (node, output, state) => ({
        ...
        props: {
          children: (
            <Text key={state.key} style={mentionStyle} onPress={() => alert('Room')}>
              {node.content}
            </Text>
          ),
        },
    ...
    customEmoji: {
      ...
      react: (node, output, state) => {
        ...
        if (emojiExtension) {
          ...
          element.props.children = <CustomEmoji key={state.key} style={style} emoji={emoji} />;
        }
        return element;
      },
    },
  ...
  return (
    <EasyMarkdown style={{ marginBottom: 0 }} rules={rules} markdownStyles={{ code: codeStyle }}>
      {msg}
    </EasyMarkdown>
  );
};
