...
const Reply = ({ attachment, timeFormat }) => {
  ...
  const renderAvatar = () => {
    ...
    return <Avatar text={attachment.author_name} size={16} avatar={attachment.author_icon} />;
  };
    ...
    return (
      <View style={styles.fieldsContainer}>
        {attachment.fields.map(field => (
          <View
            key={field.title}
            style={[styles.fieldContainer, { width: field.short ? '50%' : '100%' }]}>
    ...
    <TouchableOpacity onPress={() => onPress(attachment)} style={styles.button}>
      ...
      <View style={styles.attachmentContainer}>
        ...
        {attachment.attachments.map(attach => (
          <Reply key={attach.text} attachment={attach} timeFormat={timeFormat} />
        ))}
      </View>
    </TouchableOpacity>
  );
