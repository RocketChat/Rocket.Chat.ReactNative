...
const MessageRoomStoreProvider = ({ children, ...state }: { children: ReactNode } & MessageRoomState): ReactElement => {
	...
	// Handlers and room constants are captured once here (initial store state) and never re-synced;
	// callers must pass referentially stable values for them. Only the fields below can change mid-session
	// (e.g. an open room gets archived); the dep array keeps store writes on-change only.
	useEffect(() => {
		store.setState({
