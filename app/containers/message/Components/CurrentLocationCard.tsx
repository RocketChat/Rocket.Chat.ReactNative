...
import { mapsDeepLink, staticMapUrl, providerAttribution, providerLabel } from '../../../views/LocationShare/services/mapProviders';
...
const createStyles = (colors: TColors) =>
  StyleSheet.create({
    ...
    title: { fontSize: 16, fontWeight: '700', color: colors.fontTitlesLabels },
    coords: { fontSize: 13, color: colors.fontSecondaryInfo, marginTop: 2 },
    link: {
    ...
    mapContainer: { borderTopWidth: 1, borderBottomWidth: 1, borderColor: colors.strokeLight },
    ...
    attribution: { fontSize: 10, color: colors.fontSecondaryInfo, textAlign: 'center', paddingVertical: 6 }
  });
...
const CurrentLocationCard: React.FC<Props> = ({ msg }) => {
  ...
  const OSM_HEADERS = useMemo(
    () => ({ 'User-Agent': 'RocketChatMobile/1.0 (+https://rocket.chat) contact: mobile@rocket.chat', Referer: 'https://rocket.chat' }),
    []
  );

  const cacheKey = useMemo(() => (coords ? `osm-${coords.latitude.toFixed(5)}-${coords.longitude.toFixed(5)}-z15-v2` : undefined), [coords]);
  ...
  // Render only when coordinates are detected
  ...
  <Text style={styles.title}>📍 {I18n.t('Location')}</Text>
      ...
      {mapUrl ? (
        <Pressable onPress={onOpen} accessibilityRole='button' accessibilityLabel={openLabel} android_ripple={{ color: 'rgba(0,0,0,0.08)' }} style={({ pressed }) => [styles.mapContainer, pressed && styles.pressed]}>
          <ExpoImage source={{ uri: mapUrl, headers: OSM_HEADERS, cacheKey }} style={styles.mapImage} contentFit='cover' transition={150} cachePolicy='disk' />
      ...
      <TouchableOpacity onPress={onOpen} activeOpacity={0.7} accessibilityRole='button' accessibilityLabel={openLabel}>
        <Text style={styles.link}>🗺️ {openLabel}</Text>
      </TouchableOpacity>
    </View>
  );
};
