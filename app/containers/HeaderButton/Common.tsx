...
import { TNavigation } from '../../types/stacks';

interface IProps<T extends keyof TNavigation> { 
	navigation: StackNavigationProp<TNavigation, T>; 
	onPress: () => void; 
	testID: string;
}

// Left
export const Drawer = React.memo(({ navigation, testID, ...props }: IProps<'DirectoryView'>) => (
	<Container left>
...
export const CancelModal = React.memo(({ onPress, testID }: IProps) => (
	<Container left>
		{isIOS
...
// Right
export const More = React.memo(({ onPress, testID }: IProps) => (
	<Container>
		<Item iconName='kebab' onPress={onPress} testID={testID} />
	</Container>
));

export const Download = React.memo(({ onPress, testID, ...props }: IProps) => (
	<Container>
		<Item iconName='download' onPress={onPress} testID={testID} {...props} />
	</Container>
));

export const Preferences = React.memo(({ onPress, testID, ...props }: IProps) => (
	<Container>
		<Item iconName='settings' onPress={onPress} testID={testID} {...props} />
	</Container>
));

export const Legal = React.memo(({ navigation, testID }: IProps) => (
