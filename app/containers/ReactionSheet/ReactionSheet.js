...
const ReactionSheet = React.memo(forwardRef(({ children, theme }, ref) => {
	...
	const renderItem = useCallback(({ item }) => <UserItem user={item} theme={theme} baseUrl={data.baseUrl} userId={data.user.id} userToken={data.user.token} />);
