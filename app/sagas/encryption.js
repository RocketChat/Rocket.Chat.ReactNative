...
const handleEncryptionInit = function* handleEncryptionInit() {
	try {
		...
		const storedPrivateKey = UserPreferences.getStringAsync(`${server}-${E2E_PRIVATE_KEY}`);
		...
		const storedRandomPassword = UserPreferences.getStringAsync(`${server}-${E2E_RANDOM_PASSWORD_KEY}`);
		...
		let storedPublicKey = UserPreferences.getStringAsync(`${server}-${E2E_PUBLIC_KEY}`);
