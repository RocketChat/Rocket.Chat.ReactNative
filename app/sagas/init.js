...
const restore = function* restore() {
	try {
		const server = UserPreferences.getStringAsync(RocketChat.CURRENT_SERVER);
		let userId = UserPreferences.getStringAsync(`${RocketChat.TOKEN_KEY}-${server}`);
		...
		if (!server) {
			...
			if (servers.length > 0) {
				for (let i = 0; i < servers.length; i += 1) {
					...
					userId = UserPreferences.getStringAsync(`${RocketChat.TOKEN_KEY}-${newServer}`);
					if (userId) {
