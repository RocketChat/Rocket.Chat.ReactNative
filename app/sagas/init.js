...
const restore = function* restore() {
  try {
    ...
    if (currentServer) {
      ...
      yield put(
        actions.setAllSettings(RocketChat.parseSettings(settings.slice(0, settings.length))),
      );
      const permissions = database.objects('permissions');
      yield put(
        actions.setAllPermissions(
          RocketChat.parsePermissions(permissions.slice(0, permissions.length)),
        ),
      );
