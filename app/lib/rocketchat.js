...
const RocketChat = {
	...
	async getServerInfo(server) {
		try {
			...
			try {
				...
				if (isServerVersion(jsonRes.version, 'lowerThan', MIN_ROCKETCHAT_VERSION)) {
					return {
						success: false,
				...
				if (e.data?.error && (e.data.error === 'totp-required' || e.data.error === 'totp-invalid')) {
					...
					try {
						...
						if (loginEmailPassword) {
							...
							if (isServerVersion(serverVersion, 'greaterThanOrEqualTo', '3.9.0')) {
	...
	async getRoomMembers({ rid, allUsers, roomType, type, filter, skip = 0, limit = 10 }) {
		const serverVersion = reduxStore.getState().server.version;
		if (isServerVersion(serverVersion, 'greaterThanOrEqualTo', '3.16.0')) {
			const params = {
				roomId: rid,
				offset: skip,
				count: limit,
	...
	readThreads(tmid) {
		const serverVersion = reduxStore.getState().server.version;
		if (isServerVersion(serverVersion, 'greaterThanOrEqualTo', '3.4.0')) {
			// RC 3.4.0
