...
const RocketChat = {
	...
	async getServerInfo(server) {
		try {
			...
			try {
				...
				if (compareServerVersion(jsonRes.version, MIN_ROCKETCHAT_VERSION, 'lowerThan')) {
					return {
						success: false,
				...
				if (e.data?.error && (e.data.error === 'totp-required' || e.data.error === 'totp-invalid')) {
					...
					try {
						...
						if (loginEmailPassword) {
							...
							if (compareServerVersion(serverVersion, '3.9.0', 'greaterThanOrEqualTo')) {
	...
	async getRoomMembers({ rid, allUsers, roomType, type, filter, skip = 0, limit = 10 }) {
		const serverVersion = reduxStore.getState().server.version;
		if (compareServerVersion(serverVersion, '3.16.0', 'greaterThanOrEqualTo')) {
			const params = {
				roomId: rid,
				offset: skip,
				count: limit,
	...
	readThreads(tmid) {
		const serverVersion = reduxStore.getState().server.version;
		if (compareServerVersion(serverVersion, '3.4.0', 'greaterThanOrEqualTo')) {
			// RC 3.4.0
