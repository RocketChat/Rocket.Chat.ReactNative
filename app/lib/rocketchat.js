...
const RocketChat = {
  ...
  async testServer(url) {
    if (/^(https?:\/\/)?(((\w|[0-9-_])+(\.(\w|[0-9-_])+)+)|localhost)(:\d+)?$/.test(url)) {
      ...
      if (
        response.status === 200 &&
        response.headers.get('x-instance-id') != null &&
        response.headers.get('x-instance-id').length
      ) {
        return url;
      }
    }
  ...
  _parseUrls(urls) {
    return urls.filter(url => url.meta && !url.ignoreParse).map((url, index) => {
      ...
      tmp._id = index;
      tmp.title =
        meta.ogTitle || meta.twitterTitle || meta.title || meta.pageTitle || meta.oembedTitle;
      tmp.description =
        meta.ogDescription || meta.twitterDescription || meta.description || meta.oembedAuthorName;
      let decodedOgImage;
      if (meta.ogImage) {
  ...
  _buildMessage(message) {
    ...
    message.starred =
      message.starred &&
      (Array.isArray(message.starred) ? message.starred.length > 0 : !!message.starred);
    return message;
  },
  loadMessagesForRoom(rid, end, cb) {
    return this.ddp.call('loadHistory', rid, end, 20).then(
      data => {
  ...
  async sendFileMessage(rid, fileInfo, data) {
    ...
    try {
      ...
      const completeRresult = await RocketChat._ufsComplete(
        result.fileId,
        fileInfo.store,
        result.token,
      );
  ...
  async getRooms() {
    ...
    let lastMessage = database.objects('subscriptions').sorted('roomUpdatedAt', true)[0];
    lastMessage = lastMessage && new Date(lastMessage.roomUpdatedAt);
    let [subscriptions, rooms] = await Promise.all([
      call('subscriptions/get', lastMessage),
      call('rooms/get', lastMessage),
    ]);
  ...
  login(params, callback) {
    return this.ddp.call('login', params).then(
      result => {
  ...
  async getSettings() {
    ...
    const result = await (!temp
      ? call('public-settings/get')
      : call('public-settings/get', new Date(temp._updatedAt)));
  ...
  _filterSettings: settings =>
    settings.filter(setting => settingsType[setting.type] && setting.value),
  async getPermissions() {
    const temp = database.objects('permissions').sorted('_updatedAt', true)[0];
    const result = await (!temp
      ? call('permissions/get')
      : call('permissions/get', new Date(temp._updatedAt)));
