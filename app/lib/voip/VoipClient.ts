...
class VoipClient {
	...
	private emitter: Emitter<VoipEvents>;
	...
	public on<K extends keyof VoipEvents>(type: K, handler: Handler<VoipEvents[K]>) {
		return this.emitter.on(type, handler);
	}

	public emit<K extends keyof VoipEvents>(type: K, event?: VoipEvents[K]) {
		return this.emitter.emit(type, event as VoipEvents[K]);
	}

	public off(type: keyof VoipEvents) {
		return this.emitter.off(type);
	}
