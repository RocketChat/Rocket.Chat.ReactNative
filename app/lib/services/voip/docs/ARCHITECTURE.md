...
The OS microphone permission is **pre-acquired at session init** (`login` → `checkVoipPermission` → `preAcquireVoipMicPermission`, fire-and-forget and suppressed while a call is active), because an incoming call on a locked/backgrounded device cannot present a permission dialog.

When the permission is nonetheless denied, the **Incoming-push gate** suppresses the call natively at push receipt so the device never rings, and signals nothing to the server so the call keeps ringing on the user's other devices (**Suppress-without-ringing**). On Android this is the pure `decideIncomingVoipPushAction` routing in `VoipIncomingCallDispatch.kt` — precedence `stale → IGNORE_NO_PERMISSION → REJECT_BUSY → SHOW_INCOMING`, wired in `VoipNotification.onMessageReceived` (reads `RECORD_AUDIO`; the no-permission outcome is a log-only ignore — no Telecom, no notification, no REST). On iOS it is the PushKit handler (report-a-placeholder-then-end, no REST, no JS stash; the sub-second CallKit flash is unavoidable). The incoming **answer gate** (`answerCall` → `hasVoipCallPermission`) is demoted to a **check-only** backstop: it never prompts and ends the call silently when the mic is not currently granted, covering only revocation in the push-to-answer window — by then native has already sent the REST accept, so the call cannot be handed back to other devices. Outgoing (`startCall`) still requests in-context and alerts on denial; the alert uses the voice-call-specific `Microphone_access_needed_for_voice_calls` string, separate from the voice-message recorder. See `CONTEXT.md` for the vocabulary, `adr/0002-suppress-incoming-at-push-layer-when-mic-denied.md` for the push-layer gate, and `adr/0001-pre-acquire-microphone-at-login.md` for the pre-acquire rationale.
