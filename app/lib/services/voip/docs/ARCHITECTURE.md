...
When the permission is nonetheless denied, the **Incoming-push gate** declines the call natively at push receipt so the device never rings (**Reject-without-ringing**). On Android this is the pure `decideIncomingVoipPushAction` routing in `VoipIncomingCallDispatch.kt` — precedence `stale → REJECT_NO_PERMISSION → REJECT_BUSY → SHOW_INCOMING`, wired in `VoipNotification.onMessageReceived` (reads `RECORD_AUDIO`, maps the no-permission outcome onto the existing REST `reject`, no Telecom, no notification). On iOS it is the PushKit handler (report-a-placeholder-then-end + REST `reject`, no JS stash; the sub-second CallKit flash is unavoidable). The incoming **answer gate** (`answerCall` → `hasVoipCallPermission`) is demoted to a **check-only** backstop: it never prompts and rejects silently when the mic is not currently granted, covering only revocation in the push-to-answer window. Outgoing (`startCall`) still requests in-context and alerts on denial; the alert uses the voice-call-specific `Microphone_access_needed_for_voice_calls` string, separate from the voice-message recorder. See `CONTEXT.md` for the vocabulary, `adr/0002-reject-incoming-at-push-layer-when-mic-denied.md` for the push-layer gate, and `adr/0001-pre-acquire-microphone-at-login.md` for the pre-acquire rationale.
