...
- `VoipCallService` — `android:foregroundServiceType="microphone|phoneCall"`. Both bits are needed and serve distinct purposes:
  - `phoneCall` satisfies FGS-**start** eligibility on the incoming-accept path. That path starts the service from `VoipNotification.handleAcceptAction` *after* `connection.onAnswer()` puts the self-managed Telecom call into `STATE_ACTIVE`, which is what makes `phoneCall` startable from a non-foreground entry point (FSI / `onNewIntent`).
...
- The outgoing path also starts `VoipCallService`, invoked from JS (`MediaSessionInstance` `newCall` 'caller' branch → `NativeVoipModule.startVoipCallService(callId)`). It must be started while the initiating activity is still visible — starting `microphone` FGS from a non-foreground state throws `SecurityException`.
