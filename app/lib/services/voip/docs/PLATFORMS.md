...
If the OS microphone permission is denied, `didReceiveIncomingPushWith` declines the call before it rings: it sends the REST `reject` (`VoipService.rejectNoMicPermissionCall`) and reports a placeholder call it ends immediately — the unavoidable sub-second CallKit flash satisfies the reporting deadline above — and skips `prepareIncomingCall`, so nothing is stashed for JS (opening the app cannot resurrect the call). The read is `VoipService.hasMicrophonePermission()` (`AVAudioApplication.recordPermission` on iOS 17+, `AVAudioSession.recordPermission` earlier). A truly silent path is impossible — returning without a report kills the app. See `adr/0002-reject-incoming-at-push-layer-when-mic-denied.md`.
...
Before any of this, `VoipNotification.onMessageReceived` runs the pure `decideIncomingVoipPushAction` (precedence `stale → REJECT_NO_PERMISSION → REJECT_BUSY → SHOW_INCOMING`). If `RECORD_AUDIO` is denied it declines via REST `reject` (`rejectNoMicPermissionCall`) and returns — no PhoneAccount registration, no per-call DDP, no foreground service, no notification — so the device stays truly silent. See `adr/0002-reject-incoming-at-push-layer-when-mic-denied.md`.
...
- `VoipCallService` — `android:foregroundServiceType="microphone|phoneCall"`. Both bits are needed and serve distinct purposes:
  ...
  - `microphone` is what authorises `RECORD_AUDIO` after the app leaves the foreground — `RECORD_AUDIO` is a while-in-use permission and is revoked ~5s after backgrounding unless an FGS in the active set explicitly authorises microphone capture. `phoneCall` does not do this. See NATIVE-1178.
- The outgoing path also starts `VoipCallService`, invoked from JS (`MediaSessionInstance` `newCall` 'caller' branch → `NativeVoipModule.startVoipCallService(callId)`). It must be started while the initiating activity is still visible — starting `microphone` FGS from a non-foreground state throws `SecurityException`. The TurboModule method's `Promise<void>` is held until `VoipCallService.onStartCommand` actually returns from `startForeground` and signals back via `VoipModule.notifyFgsStarted` / `notifyFgsFailed`, so async failures (`ForegroundServiceTypeNotAllowedException`, `ForegroundServiceDidNotStartInTimeException`) reach JS instead of being masked by a synchronous `resolve(null)`. A 7-second timeout rejects with `E_VOIP_FGS_TIMEOUT` if the service never reports back. On any rejection JS tears down the outgoing call (`endCall` + user-visible alert) and skips navigation to `CallView`, since `endCall` detaches the `'ended'` listener that would otherwise have fired `Navigation.back()`.
- `react-native-callkeep`'s own `VoiceConnectionService` declares `microphone|phoneCall` in its manifest entry too, but Telecom does not promote a self-managed `ConnectionService` to a foreground service on its own — `VoipCallService` is the only FGS that actually runs.
