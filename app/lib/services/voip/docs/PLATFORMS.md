...
- The outgoing path also starts `VoipCallService`, invoked from JS (`MediaSessionInstance` `newCall` 'caller' branch → `NativeVoipModule.startVoipCallService(callId)`). It must be started while the initiating activity is still visible — starting `microphone` FGS from a non-foreground state throws `SecurityException`. The TurboModule method returns `Promise<void>`; if the start is refused, JS tears down the outgoing call (`endCall` + user-visible alert) instead of continuing without an FGS.
