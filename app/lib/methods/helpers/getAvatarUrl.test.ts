...
describe('formatUrl function', () => {
	...
	test('formats an external provider URI to get the user avatar', () => {
		const url = 'https://chat.cortexflex.org/avatar/reinaldoneto';
		const size = 30;
		const query = undefined;
		const expected = 'https://chat.cortexflex.org/avatar/reinaldoneto?format=png&size=30';
		const result = formatUrl(url, size, query);
		expect(result).toEqual(expected);
	});

	test('formats an external provider URI that already includes a query to get the user avatar', () => {
		const url = 'https://chat.cortexflex.org/avatar?rcusername=reinaldoneto';
		const size = 30;
		const query = undefined;
		const expected = 'https://chat.cortexflex.org/avatar?rcusername=reinaldoneto&format=png&size=30';
		const result = formatUrl(url, size, query);
		expect(result).toEqual(expected);
	});
});
