...
export function subscribeUsersPresence() {
	...
	if (compareServerVersion(serverVersion, '1.1.0', 'lowerThan')) {
...
export default async function getUsersPresence() {
	...
	if (compareServerVersion(serverVersion, '1.1.0', 'greaterThanOrEqualTo')) {
		let params = {};

		// if server is greather than or equal 3.0.0
		if (compareServerVersion(serverVersion, '3.0.0', 'greaterThanOrEqualTo')) {
			// if not have any id
			if (!ids.length) {
				return;
			}
