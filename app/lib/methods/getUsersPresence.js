...
export function subscribeUsersPresence() {
	...
	if (isServerVersion(serverVersion, 'lowerThan', '1.1.0')) {
...
export default async function getUsersPresence() {
	...
	if (isServerVersion(serverVersion, 'greaterThanOrEqualTo', '1.1.0')) {
		let params = {};

		// if server is greather than or equal 3.0.0
		if (isServerVersion(serverVersion, 'greaterThanOrEqualTo', '3.0.0')) {
			// if not have any id
			if (!ids.length) {
				return;
			}
