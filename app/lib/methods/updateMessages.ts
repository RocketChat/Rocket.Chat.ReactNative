...
export default async function updateMessages({
	...
	return db.write(async () => {
		...
		const filteredMsgsToUpdate = allMessagesRecords.filter(i1 => update.find(i2 => i1.id === i2._id && i1._updatedAt < i2._updatedAt));
		...
		const filteredThreadsToUpdate = allThreadsRecords.filter(i1 => allThreads.find(i2 => i1.id === i2._id && i1._updatedAt < i2._updatedAt));
		...
		const filteredThreadMessagesToUpdate = allThreadMessagesRecords.filter(i1 => allThreadMessages.find(i2 => i1.id === i2._id && i1._updatedAt < i2._updatedAt));
