...
On **iOS** there is no native counterpart: `enrollProbe`/`disenrollProbe` are no-ops and `isEnrollmentValid()` resolves `true`, because the sentinel deletion already covers enrollment changes for free. The iOS `enrollProbe` fallback resolves `false` (there is no key to bind), so `enroll()`'s fail-closed check is guarded by `isAndroid` — dropping that guard would make enabling biometry impossible on every iOS device.
