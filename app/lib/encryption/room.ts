...
import { e2eGetUsersOfRoomWithoutKey, e2eRequestRoomKey, e2eSetRoomKeyID, e2eUpdateGroupKey } from '../services';
...
export default class EncryptionRoom {
	...
	encryptRoomKeyForUser = async (user: Pick<IUser, '_id' | 'e2e'>) => {
		if (user?.e2e?.public_key) {
			...
			await e2eUpdateGroupKey(user?._id, this.roomId, this.keyID + encryptedUserKey);
		}
	};
