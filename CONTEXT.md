...
Not every `_updatedAt` is worth the same. A Message's `_updatedAt` read out of a **server response** is server truth, and is the only legitimate source for the sync cursor. The same field read off a **WatermelonDB row** is device-tainted: offline sends, Temp and Error sends, push-inserted rows, and `normalizeMessage`'s `_updatedAt || new Date()` fallback all stamp the device clock.

Therefore the **Sync Watermark** must be taken from the raw payload _before_ `normalizeMessage` / `buildMessage` runs — never from a database row, and never from `Date.now()`.

| Term                 | Definition                                                                                                                   | Aliases to avoid           |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| **Sync Watermark**   | The fetch cursor for a Subscription (`lastOpen` column): the newest server `_updatedAt` actually received for that Room      | Last open, last update     |
| **Last Seen**        | The Subscription's read receipt (`ls`): the newest Message the user has read, which anchors the Unread Separator             | Last open, last read       |
...
| **Lying Cursor**     | A Sync Watermark ahead of the Room's newest Server Timestamp, so every sync drains empty and the Gap never closes            | Stale cursor, bad lastOpen |

A **Sync Watermark** and a **Last Seen** are not interchangeable — conflating them (one column serving as both fetch cursor and unread anchor) is what produced permanently invisible Messages.
...
- **"`lastOpen`"** names a database column, not a concept: it stores the **Sync Watermark**, a server-clock fetch cursor. It has never meant "when the user last opened the room". The Unread Separator anchor is **Last Seen** (`ls`). Do not read `lastOpen` as a read receipt or write a device clock into it.
