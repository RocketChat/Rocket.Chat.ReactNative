...
| Term                | Definition                                                                                                                                                                                                                                                                                                                    | Aliases to avoid       |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| **Sync Cursor**     | The newest server-stamped Message `_updatedAt` this device has fetched for a Room, stored on the Subscription as `lastOpen`; computed from the raw server response payload before `normalizeMessage` runs — never from a WatermelonDB row, never from `Date.now()`. Used as the cursor for the next `chat.syncMessages` call. | Cursor, watermark      |
| **Last Seen**       | The `_updatedAt` of the last Message the user has read in a Room, stored on the Subscription as `ls`; surfaced in RoomView as `state.lastSeen` to position the Unread Separator.                                                                                                                                              | Read marker            |
...
Therefore the sync cursor (`subscription.lastOpen`) and any sync watermark must be computed from the raw response payload before `normalizeMessage` runs — never from a DB row, never from `Date.now()`. A skewed device clock silently poisons watermarks derived locally.
