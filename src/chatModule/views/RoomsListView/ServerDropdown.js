...
export default class ServerDropdown extends Component {
    ...
    >
      <View style={styles.serverItemContainer}>
        {item.iconURL ? (
          ...
          <Image source={{ uri: "logo" }} style={styles.serverIcon} />
        )}
        <View style={styles.serverTextContainer}>
          <Text style={styles.serverName}>{item.name || item.id}</Text>
          <Text style={styles.serverUrl}>{item.id}</Text>
        </View>
        {item.id === this.props.server ? (
          <Image style={styles.checkIcon} source={{ uri: "check" }} />
        ) : null}
      </View>
    </Touch>
  );
  ...
  render() {
    const maxRows = 4;
    const initialTop =
      41 + Math.min(this.state.servers.length, maxRows) * ROW_HEIGHT;
