...
export default class UploadProgress extends Component {
  ...
  constructor(props) {
    ...
    // this.uploads = database
    //   .objects("uploads", )
    //   .filtered("rid = $0", this.props.rid);
    // this.uploads.addListener(this.updateUploads);
  }

  async componentWillMount() {
    // this.uploads = await database.objects(
    //   "uploads",
    //   `WHERE rid=${this.props.rid}`
    // );
    // this.uploads.addListener(this.updateUploads);
  }
  ...
  async componentDidMount() {
    ...
    this.uploads.forEach(u => {
      if (!RocketChat.isUploadActive(u.path)) {
        ...
        // });
        const [upload] = database.objects("uploads", `WHERE path = ${u.path}`);
        if (upload) {
          upload.error = true;
        }
      }
    ...
    if (!item.error) {
      return [
        <View key="row" style={styles.row}>
          ...
          <Text
            style={[styles.descriptionContainer, styles.descriptionText]}
            ellipsizeMode="tail"
            numberOfLines={1}
          >
          ...
          <Icon
            ...
            size={20}
            color="#9EA2A8"
            onPress={() => this.cancelUpload(item)}
          />
        </View>,
        <View
          key="progress"
          style={[
            styles.progress,
            { width: (this.props.window.width * item.progress) / 100 }
          ]}
        />
      ];
    }
